...
export async function ensureSchema() {
  ...
  // Chunked upload tables (for robust, stateless uploads)
  await query(
...
export async function createUploadSession({ id, fileName, fileType, fileSize, topicId, title }) {
  await ensureSchema();
  await query(
    `INSERT INTO upload_sessions (id, file_name, file_type, file_size, topic_id, title)
     VALUES ($1,$2,$3,$4,$5,$6)`,
    [id, fileName, fileType || null, fileSize || null, topicId, title]
  );
}
...
export async function getUploadSession(id) {
  await ensureSchema();
  const res = await query(
    `SELECT id, file_name, file_type, file_size, topic_id, title, created_at FROM upload_sessions WHERE id = $1`,
    [id]
  );
  return res.rows[0] || null;
}

export async function addUploadChunk(uploadId, chunkIndex, buffer) {
  await ensureSchema();
  await query(
...
export async function listUploadChunks(uploadId) {
  await ensureSchema();
  const res = await query(
    `SELECT chunk_index, data FROM upload_chunks WHERE upload_id = $1 ORDER BY chunk_index ASC`,
    [uploadId]
  );
  return res.rows; // [{chunk_index, data: Buffer}]
}

export async function deleteUploadSession(uploadId) {
  await ensureSchema();
  await query(`DELETE FROM upload_sessions WHERE id = $1`, [uploadId]);
}
