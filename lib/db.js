...
export async function ensureSchema() {
  ...
  await query(`INSERT INTO owners (slug, name, sort_order) VALUES ('default', '기본', 0) ON CONFLICT (slug) DO NOTHING;`);
...
export async function listTopicsWithMaterials(ownerSlug = 'default') {
  ...
  const topicsRes = await query(
    `SELECT id, title, sort_order, created_at FROM topics WHERE owner_id = $1 ORDER BY sort_order ASC, id ASC`,
    [ownerId]
  );
